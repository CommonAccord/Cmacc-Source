Def.=[F/US/00/Agt/Acquire/Shares/MSPA/SPA/Def/0.md]
...
That.Div=The parties agree as follows:

Model.Root={Doc}{Annex.Div}  

Doc={Frame.Head.Div}{This.Div}{Why.Div}<br><br>{That.Div}<br>{Text.Div}<br>{Frame.By.Div}

Frame.Head.Div={Head.Message}<center><h4>{Doc.Ti}</h4><br>Effective Date: {EffectiveDate.YMD}</center><br><br>

Head.Message=<font color="grey">GUID: {Doc.GUID}</font>

Frame.By.Div=<br>{PageBreak}<br>{By.0.sec}<br><br><table><tr><td valign="top" width="300px"><hr>{Buyer.US.Frame.Sign.Block}</td><td width="100px"></td><td valign="top" width="300px"><hr>{Seller.Sign.Block}</td></tr></table><br>

By.0.sec=IN WITNESS WHEREOF, the parties have executed {_this_Agreement} as of {EffectiveDate.YMD}.

This.Div=<br>{This.sec}<br>

Why.Div=<br><br><b>{Why.Ti}</b><br><ul type="none"><li>{Why.secs}</li></ul>

Text.Div={sec}

Annex.Div=<hr><hr><center><h4>{Annex.Ti}</h4></center><br>{Annex.sec}
